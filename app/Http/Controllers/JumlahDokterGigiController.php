...
class JumlahDokterGigiController extends Controller
{
    ...
    {
        $datas = SdmDokterGigi::orderBy('updated_at', 'desc')->get();
        ...
        return view('layers.jumlah-dokter-gigi.index',["datas"=>$datas, 'active'=>['sdm', 'dokter_gigi'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = SdmDokterGigi::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new SdmDokterGigi();
            ...
            $updatedData = SdmDokterGigi::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/dokter-gigi');   
    }

   /**
     * Remove the specified resource from storage.
     */
    public function destroy(SdmDokterGigi $id)
    {
        $id->delete();
