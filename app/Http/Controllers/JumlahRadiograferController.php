...
class JumlahRadiograferController extends Controller
{
    ...
    {
        $datas = SdmRadiografer::orderBy('updated_at', 'desc')->get();
        $lastUpdate = $datas->count() ? Carbon::parse($datas->first()['updated_at'])->format('Y-m-d') : '2000-01-01';

        $getStatusPengiriman = StatusPengiriman::where('jenis_data', 'Jumlah Radiografer')->first();
        ...
        return view('layers.jumlah-radiografer.index',["datas"=>$datas, 'active'=>['sdm', 'radiografer'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = SdmRadiografer::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new SdmRadiografer();
            ...
            $updatedData = SdmRadiografer::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/radiografer');   
    }

   /**
     * Remove the specified resource from storage.
     */
    public function destroy(SdmRadiografer $id)
    {
        $id->delete();
