...
use App\Models\IktWaktuTanggapKomplain;
...
class KecepatanWaktuTanggapKomplainController extends Controller
{
    ...
    {
        $datas = IktWaktuTanggapKomplain::orderBy('updated_at', 'desc')->get();
        $lastUpdate = $datas->count() ? Carbon::parse($datas->first()['updated_at'])->format('Y-m-d') : '2000-01-01';

        $getStatusPengiriman = StatusPengiriman::where('jenis_data', 'Kecepatan Waktu Tanggap Komplain')->first();
        ...
        return view('layers.kecepatan-waktu-tanggap-komplain.index',["datas"=>$datas, 'active'=>['ikt', 'kecepatan_waktu_tanggap_komplain'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktWaktuTanggapKomplain::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktWaktuTanggapKomplain();
            ...
            $updatedData = IktWaktuTanggapKomplain::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/kecepatan-waktu-tanggap-komplain');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktWaktuTanggapKomplain $id)
    {
        $id->delete();
