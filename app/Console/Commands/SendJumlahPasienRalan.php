...
class SendJumlahPasienRalan extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:jumlah-pasien-ralan';
     ...
     * @var string
     */
    protected $description = 'Mengirim data jumlah pasien rawat jalan/poli';
    ...
    public function handle()
    {
        ...
        $datas = LayananJumlahPasienRalan::whereDate('tgl_transaksi', $targetDate)->get();
        $statusPengiriman = [];
        ...
        foreach ($datas as $data) {
            ...
            $response = Http::withHeaders(['token' => $token])->post('Https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/layanan/pasien_ralan_poli', $data);
            ...
            $log->modul = 'Layanan';
            $log->jenis_data = 'Jumlah Pasien Rawat Jalan/Poli';
            $log->tgl_transaksi = $data['tgl_transaksi'];
            $log->kata_kunci = 'Poli: ' . $data['nama_poli'];
            $log->status = $status;
            $log->pesan = $message;
            $log->eror = $errors;
        ...
        $selectedData = StatusPengiriman::Where('jenis_data', 'Jumlah Pasien Rawat Jalan/Poli')->firstOrFail();
