...
class JumlahDokterGigiController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $getData = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/get/data/kesehatan/sdm/dokter_gigi');
        ...
        $filterNotifikasi = Notifikasi::where('submenu', 'Dokter Gigi')->orderBy('updated_at', 'DESC')->get();
        ...
        return view('layers.jumlah-dokter-gigi.index',["datas"=>$datas, 'active'=>['sdm', 'dokter_gigi'], "updateStatus"=>$updateStatus, "lastUpdate"=>$lastUpdate, 'notifikasi' => $notifikasi, 'updateDatetime' => $updateDatetime, 'savedData' => session('savedData')]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $getToken = Http::post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/token',[
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/sdm/dokter_gigi', $validatedData);
        ...
        if ($response->successful()) { 
            if ($errorResponse) {
                ...
                $savedData = true;
                return Redirect::to('/dokter-gigi')->with('savedData', $savedData);  
            }
        } else {
            $errorLists = [];
