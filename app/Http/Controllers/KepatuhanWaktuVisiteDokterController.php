...
use App\Models\IktKepatuhanWaktuVisiteDokter;
...
class KepatuhanWaktuVisiteDokterController extends Controller
{
    ...
    {
        $datas = IktKepatuhanWaktuVisiteDokter::orderBy('updated_at', 'desc')->get();
        $lastUpdate = $datas->count() ? Carbon::parse($datas->first()['updated_at'])->format('Y-m-d') : '2000-01-01';

        $getStatusPengiriman = StatusPengiriman::where('jenis_data', 'Kepatuhan Waktu Visite Dokter')->first();
        ...
        return view('layers.kepatuhan-waktu-visite-dokter.index',["datas"=>$datas, 'active'=>['ikt', 'kepatuhan_waktu_visite_dokter'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    {
        return view('layers.kepatuhan-waktu-visite-dokter.form',['active'=>['ikt', 'kepatuhan_waktu_visite_dokter']]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktKepatuhanWaktuVisiteDokter::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktKepatuhanWaktuVisiteDokter();
            ...
            $updatedData = IktKepatuhanWaktuVisiteDokter::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/kepatuhan-waktu-visite-dokter');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktKepatuhanWaktuVisiteDokter $id)
    {
        $id->delete();
