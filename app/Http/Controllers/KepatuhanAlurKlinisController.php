...
use App\Models\IktKepatuhanAlurKlinis;
...
class KepatuhanAlurKlinisController extends Controller
{
    ...
    {
        $datas = IktKepatuhanAlurKlinis::orderBy('updated_at', 'desc')->get();
        $lastUpdate = $datas->count() ? Carbon::parse($datas->first()['updated_at'])->format('Y-m-d') : '2000-01-01';

        $getStatusPengiriman = StatusPengiriman::where('jenis_data', 'Kepatuhan Terhadap Alur Klnis (Clinical Pathway)')->first();
        ...
        return view('layers.kepatuhan-alur-klinis.index',["datas"=>$datas, 'active'=>['ikt', 'kepatuhan_alur_klinis'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    {
        return view('layers.kepatuhan-alur-klinis.form',['active'=>['ikt', 'kepatuhan_alur_klinis']]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktKepatuhanAlurKlinis::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktKepatuhanAlurKlinis();
            ...
            $updatedData = IktKepatuhanAlurKlinis::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/kepatuhan-alur-klinis');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktKepatuhanAlurKlinis $id)
    {
        $id->delete();
