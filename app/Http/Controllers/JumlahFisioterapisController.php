...
class JumlahFisioterapisController extends Controller
{
    ...
    public function index()
    {
        ...
        $getData = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/get/data/kesehatan/sdm/fisioterapis');
        ...
        $filterNotifikasi = Notifikasi::where('submenu', 'Fisioterapis')->orderBy('updated_at', 'DESC')->get();
        ...
        return view('layers.jumlah-fisioterapis.index',["datas"=>$datas, 'active'=>['sdm', 'fisioterapis'], "updateStatus"=>$updateStatus, "lastUpdate"=>$lastUpdate, 'notifikasi' => $notifikasi, 'updateDatetime' => $updateDatetime, 'savedData' => session('savedData')]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $getToken = Http::post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/token',[
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/sdm/fisioterapis', $validatedData);
        ...
        if ($response->successful()) { 
            if ($errorResponse) {
                ...
                $savedData = true;
                return Redirect::to('/fisioterapis')->with('savedData', $savedData);  
            }
        } else {
            $errorLists = [];
