...
class JumlahRadiograferController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $getData = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/get/data/kesehatan/sdm/radiographer');
        ...
        $filterNotifikasi = Notifikasi::where('submenu', 'Radiografer')->orderBy('updated_at', 'DESC')->get();
        ...
        return view('layers.jumlah-radiografer.index',["datas"=>$datas, 'active'=>['sdm', 'radiografer'], "updateStatus"=>$updateStatus, "lastUpdate"=>$lastUpdate, 'notifikasi' => $notifikasi, 'updateDatetime' => $updateDatetime, 'savedData' => session('savedData')]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $getToken = Http::post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/token',[
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/sdm/radiographer', $validatedData);
        ...
        if ($response->successful()) { 
            if ($errorResponse) {
                ...
                $savedData = true;
                return Redirect::to('/radiografer')->with('savedData', $savedData);  
            }
        } else {
            $errorLists = [];
