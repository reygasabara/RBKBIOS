...
class SendSaldoPengelolaanKas extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:saldo-pengelolaan-kas';
     ...
     * @var string
     */
    protected $description = 'Mengirim data saldo rekening pengelolaan kas (keuangan)...';
    ...
    {   
        ...
        $datas = SaldoPengelolaanKas::whereDate('tgl_transaksi', $targetDate)->get();
        $statusPengiriman = [];
        ...
        foreach ($datas as $data) {
            ...
            $response = Http::withHeaders(['token' => $token])->post('Https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/keuangan/saldo/saldo_pengelolaan_kas', $data);
            ...
            $log->modul = 'Keuangan';
            $log->jenis_data = 'Saldo Rekening - Pengelolaan Kas';
            $log->tgl_transaksi = $data['tgl_transaksi'];
            $log->kata_kunci = 'No. bilyet: ' . $data['no_bilyet'];
            $log->status = $status;
            $log->pesan = $message;
            $log->eror = $errors;
        ...
        $selectedData = StatusPengiriman::Where('jenis_data', 'Saldo Rekening - Pengelolaan Kas')->firstOrFail();
