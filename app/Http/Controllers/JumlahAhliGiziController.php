...
class JumlahAhliGiziController extends Controller
{
    ...
    {
        $datas = SdmAhliGizi::orderBy('updated_at', 'desc')->get();
        $lastUpdate = $datas->count() ? Carbon::parse($datas->first()['updated_at'])->format('Y-m-d') : '2000-01-01';

        $getStatusPengiriman = StatusPengiriman::where('jenis_data', 'Jumlah Ahli Gizi')->first();
        ...
        return view('layers.jumlah-ahli-gizi.index',["datas"=>$datas, 'active'=>['sdm', 'ahli_gizi'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = SdmAhliGizi::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new SdmAhliGizi();
            ...
            $updatedData = SdmAhliGizi::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/ahli-gizi');   
    }

   /**
     * Remove the specified resource from storage.
     */
    public function destroy(SdmAhliGizi $id)
    {
        $id->delete();
