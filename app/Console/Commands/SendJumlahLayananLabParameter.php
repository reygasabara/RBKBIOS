...
class SendJumlahLayananLabParameter extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:jumlah-layanan-lab-parameter';
     ...
     * @var string
     */
    protected $description = 'Mengirim data jumlah layanan laboratorium (parameter) ...';
    ...
    public function handle()
    {
        ...
        $datas = LayananLabParameter::whereDate('tgl_transaksi', $targetDate)->get();
        $statusPengiriman = [];
        ...
        foreach ($datas as $data) {
            ...
            $response = Http::withHeaders(['token' => $token])->post('Https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/layanan/laboratorium_detail', $data);
            ...
            $log->modul = 'Layanan';
            $log->jenis_data = 'Jumlah Layanan Laboratorium (Parameter)';
            $log->tgl_transaksi = $data['tgl_transaksi'];
            $log->kata_kunci = 'Layanan: ' . $data['nama_layanan'];
            $log->status = $status;
            $log->pesan = $message;
            $log->eror = $errors;
        ...
        $selectedData = StatusPengiriman::Where('jenis_data', 'Jumlah Layanan Laboratorium (Parameter)')->firstOrFail();
