...
use App\Models\IktKepatuhanIdentifikasiPasien;
...
class KepatuhanIdentifikasiPasienController extends Controller
{
    ...
    {
        $datas = IktKepatuhanIdentifikasiPasien::orderBy('updated_at', 'desc')->get();
        ...
        return view('layers.kepatuhan-identifikasi-pasien.index',["datas"=>$datas, 'active'=>['ikt', 'kepatuhan_identifikasi_pasien'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    {
        return view('layers.kepatuhan-identifikasi-pasien.form',['active'=>['ikt', 'kepatuhan_identifikasi_pasien']]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktKepatuhanIdentifikasiPasien::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktKepatuhanIdentifikasiPasien();
            ...
            $updatedData = IktKepatuhanIdentifikasiPasien::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/kepatuhan-identifikasi-pasien');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktKepatuhanIdentifikasiPasien $id)
    {
        $id->delete();
