...
class JumlahApotekerController extends Controller
{
    ...
    public function index()
    {
        ...
        $getData = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/get/data/kesehatan/sdm/pharmacist');
        ...
        $filterNotifikasi = Notifikasi::where('submenu', 'Apoteker')->orderBy('updated_at', 'DESC')->get();
        ...
        return view('layers.jumlah-apoteker.index',["datas"=>$datas, 'active'=>['sdm', 'apoteker'], "updateStatus"=>$updateStatus, "lastUpdate"=>$lastUpdate, 'notifikasi' => $notifikasi, 'updateDatetime' => $updateDatetime, 'savedData' => session('savedData')]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $getToken = Http::post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/token',[
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/sdm/pharmacist', $validatedData);
        ...
        if ($response->successful()) { 
            if ($errorResponse) {
                ...
                $savedData = true;
                return Redirect::to('/apoteker')->with('savedData', $savedData);  
            }
        } else {
            $errorLists = [];
