...
class KepatuhanAlurKlinisController extends Controller
{
    ...
    public function index()
    {
        ...
        $dataKepatuhanAlurKlinis = Http::withHeaders(['token' => $token])->post('https://training-bios2.kemenkeu.go.id/api/get/data/kesehatan/ikt/kepatuhan_alur_klinis');
    ...
    public function store(Request $request)
    {
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://training-bios2.kemenkeu.go.id/api/ws/kesehatan/ikt/kepatuhan_alur_klinis', $validatedData);
        ...
        if ($response->successful()) { 
            if ($errorResponse) {
                ...
                $savedData = true;
                return Redirect::to('/kepatuhan-alur-klinis')->with('savedData', $savedData);
            }
        } else {
            $errorLists = [];
