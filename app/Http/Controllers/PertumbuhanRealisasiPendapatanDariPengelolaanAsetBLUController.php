...
use App\Models\IktPendapatanAsetBlu;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Redirect;
use RealRashid\SweetAlert\Facades\Alert;

class PertumbuhanRealisasiPendapatanDariPengelolaanAsetBLUController extends Controller
{
    /**
    ...
    {
        $datas = IktPendapatanAsetBlu::orderBy('updated_at', 'desc')->get();
        ...
        return view('layers.pendapatan-aset-blu.index',["datas"=>$datas, 'active'=>['ikt', 'pendapatan_aset_blu'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    {
        return view('layers.pendapatan-aset-blu.form',['active'=>['ikt', 'pendapatan_aset_blu']]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktPendapatanAsetBlu::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktPendapatanAsetBlu();
            ...
            $updatedData = IktPendapatanAsetBlu::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/pertumbuhan-pendapatan-pengelolaan-aset-blu');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktPendapatanAsetBlu $id)
    {
        $id->delete();
