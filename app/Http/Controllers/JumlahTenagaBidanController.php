...
class JumlahTenagaBidanController extends Controller
{
    ...
    public function index()
    {
        ...
        $getData = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/get/data/kesehatan/sdm/bidan');
        ...
        $filterNotifikasi = Notifikasi::where('submenu', 'Bidan')->orderBy('updated_at', 'DESC')->get();
        ...
        return view('layers.jumlah-tenaga-bidan.index',["datas"=>$datas, 'active'=>['sdm', 'bidan'], "updateStatus"=>$updateStatus, "lastUpdate"=>$lastUpdate, 'notifikasi' => $notifikasi, 'updateDatetime' => $updateDatetime, 'savedData' => session('savedData')]);
    }
    ...
    public function store(Request $request)
    {
       ...
       $getToken = Http::post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/token',[
        ...
        $response = Http::withHeaders(['token' => $token])->post('https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/sdm/bidan', $validatedData);
