...
class SendJumlahTindakanOperasi extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:jumlah-tindakan-operasi';
     ...
     * @var string
     */
    protected $description = 'Mengirim data jumlah tindakan operasi';
    ...
    public function handle()
    {
        ...
        $datas = LayananTindakanOperasi::whereDate('tgl_transaksi', $targetDate)->get();
        $statusPengiriman = [];
        ...
        foreach ($datas as $data) {
            ...
            $response = Http::withHeaders(['token' => $token])->post('Https://' . env('DOMAIN_NAME') . '.kemenkeu.go.id/api/ws/kesehatan/layanan/operasi', $data);
            ...
            $log->modul = 'Layanan';
            $log->jenis_data = 'Jumlah Tindakan Operasi';
            $log->tgl_transaksi = $data['tgl_transaksi'];
            $log->kata_kunci = 'klasifikasi operasi: ' . $data['klasifikasi_operasi'];
            $log->status = $status;
            $log->pesan = $message;
            $log->eror = $errors;
        ...
        $selectedData = StatusPengiriman::Where('jenis_data', 'Jumlah Tindakan Operasi')->firstOrFail();
