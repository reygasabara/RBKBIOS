...
use App\Models\IktVisitePertama;
...
class JumlahKegiatanVisitePasienPertamaController extends Controller
{
    ...
    {
        $datas = IktVisitePertama::orderBy('updated_at', 'desc')->get();
        ...
        return view('layers.visite-pertama.index',["datas"=>$datas, 'active'=>['ikt', 'visite_pertama'], "updated"=>$updated, "lastUpdate"=>$lastUpdate]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $checkedData = IktVisitePertama::Where('tgl_transaksi', $validatedData['tgl_transaksi'])->first();
        $status = $checkedData ? 'update' : 'add';

        if ($status == 'add') {
            $data = new IktVisitePertama();
            ...
            $updatedData = IktVisitePertama::where('tgl_transaksi', $validatedData['tgl_transaksi'])->firstOrFail();
        ...
        return Redirect::to('/visite-pertama');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(IktVisitePertama $id)
    {
        $id->delete();
